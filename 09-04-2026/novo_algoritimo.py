...
# --- PONTO CRUCIAL: Capturamos o dia 08/04 ANTES do dropna ---
features = ['Price', 'Open', 'High', 'Low', 'SMA_5', 'Retorno_Hoje']
# Pegamos a última linha (que é o pregão de ontem, 08/04)
...
print("--- RELATÓRIO: REGRESSÃO LINEAR (09/04) ---")
...
# Usamos os dados isolados de 08/04 para prever a variação de hoje
variacao_prevista = model.predict(dados_ontem)[0]
preco_ontem = dados_ontem['Price'].values[0]

print(f"\nFechamento de Ontem (08/04): R$ {preco_ontem:.2f}")
print(f"Variação esperada para hoje: R$ {variacao_prevista:+.2f}")
print(f"--- PREVISÃO PARA 09/04: R$ {preco_ontem + variacao_prevista:.2f} ---")